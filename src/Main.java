import java.util.Random;

public class Main {

    public static int rolador(int lados) {
        Random rd = new Random();
        return rd.nextInt(1, lados);
    }

    public static void main(String[] args) {
        System.out.println("Olá! Você tirou " + rolador(100000));
        System.out.println("Taca a mãe pra ver se quica glenn.");
        System.out.println(rolador(100));
        System.out.println(rolador(2));
        System.out.println(rolador(4));
        System.out.println(rolador(6));
        System.out.println(rolador(8));
        int[] partes = new int[60];

        
    }
}
